					...
					<div class="order_data_column">
						...
						<div class="address">
							...
							if ( $order->get_formatted_shipping_address() ) {?>
								<div class="copy-shiping">
									<div class="shipping-adress">
										<?php echo 'Name: '.$order->billing_first_name.' '.$order->billing_last_name.' - Address: '.$order->billing_address_1.' '. $order->billing_address_2.', '.$order->billing_city.', '.$order->billing_state.'('.$order->billing_postcode.'), '.$order->billing_country.' - Phone: '.$order->billing_phone; ?>
									</div>
									<a href="javascript:void(0)" class="icon-copy"></a>
								</div>
							<?php } else {
						...
						<div class="shipping_method_top">
							<?php
								$TypeMethodShipping = reset( $order->get_items( 'shipping' ) )->get_instance_id();
								if($TypeMethodShipping == 1){
									$ClassMethodShipping = 'free_shipping';
								}elseif($TypeMethodShipping == 2){
									$ClassMethodShipping = 'rate_2';
								}elseif($TypeMethodShipping == 3){
									$ClassMethodShipping = 'rate_3';
								}elseif($TypeMethodShipping == 4){
									$ClassMethodShipping = 'rate_4';
								}
							?>
							<h3>Shipping Method</h3>
							<p class="<?php echo $ClassMethodShipping; ?>"><?php echo $order->get_shipping_method(); ?></p>
						</div>
